...
// Transformando em funções globais para o HTML enxergar
window.digitar = function(numero) {
...
function verificarSenha() {
    if (senhaDigitada === senhaCorreta) {
        telaBloqueio.style.transition = "opacity 0.5s ease";
...
document.addEventListener('keydown', function(event) {
    // 1. Verifica se a tela de bloqueio ainda está ativa. 
    // Se ela já sumiu (display = "none"), nós ignoramos o teclado para não atrapalhar a digitação das cartas.
    const telaBloqueio = document.getElementById('tela-bloqueio');
    if (telaBloqueio.style.display === "none") {
        return; 
    }

    // 2. Captura os números de '0' a '9' (teclado numérico ou superior)
    if (event.key >= '0' && event.key <= '9') {
        window.digitar(event.key);
    }
    // 3. Captura a tecla 'Backspace' para apagar o último número
    else if (event.key === 'Backspace') {
        window.apagarUltimo();
    }
    // 4. Captura a tecla 'Escape' (Esc) ou a letra 'C' para limpar tudo
